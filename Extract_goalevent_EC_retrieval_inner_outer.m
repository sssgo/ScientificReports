function [goal] = Extract_goalevent_EC_retrieval_inner_outer(behInfo,slope,intercept,percent)
% wwj 2019-5-27 
 ...
 %   samplerate=round(behaviorsamplerate/0.01);
   ...
   % envelope_of_signal=abs(hilbert(speed));
    ...
    gridevent(:,3:4)=([slope*gridevent(:,3)+intercept,slope*gridevent(:,4)+intercept]);% ио
...
POST=goalevent(:,find(goalevent(10,:)<=cutdroperror2));
   ...
   center_cut1=prctile(goalevent(12,:),67);
   center_cut2=prctile(goalevent(12,:),33);
   
   goodobjnear=goodobj(:,find(goodobj(13,:)<=center_cut2));
   goodobjmid=goodobj(:,find(goodobj(13,:)<center_cut1 & goodobj(13,:)>center_cut2));
   goodobjfar=goodobj(:,find(goodobj(13,:)>center_cut1));
   
   badobjnear=badobj(:,find(badobj(13,:)<=center_cut2));
   badobjmid=badobj(:,find(badobj(13,:)<center_cut1 & badobj(13,:)>center_cut2));
   badobjfar=badobj(:,find(badobj(13,:)>center_cut1));
